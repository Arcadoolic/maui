...
- `Config.class.ts` — loads/saves `mame-awesome-ui-config.json` (mame path, mame binary filename, avatars path, MAUI gamepad binding overrides).
...
- `Gamepads.class.ts` + `src/composables/useControllable.ts` — gamepad polling dispatched as `gamepadKeydown`/`gamepadKeyup` window `CustomEvent`s; `useControllable()` is a composable that components call to register combined keyboard and gamepad handlers (`const {onKeydown, onKeyup} = useControllable()`), with cleanup handled automatically in `onUnmounted`. The gamepad-to-key mapping is `src/assets/controllers.json` plus the overrides saved by the BO's MAUI > Controls capture in the config JSON (`mauiControls`, merged by `MauiControls.ts`); `Gamepads` reloads them when the file changes (`fs.watchFile`). The capture itself is the one main→renderer call in the app: the BO (main process) calls `win.webContents.executeJavaScript('window.mauiCapturePress(...)')` from `background.ts`, since only the cabinet window's Gamepad API sees the pads.
