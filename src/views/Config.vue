...
<script lang="ts">
    ...
    export default class Config extends Vue {
        ...
        public async selectMamePath() {
            let mameBinaryNames = ['mame.exe', 'mame64.exe', 'mame'];

            let mameDirPath = await remote.dialog.showOpenDialog({
                title: 'Select mame binary path',
                properties: ['openDirectory', 'showHiddenFiles'],
            });

            if (mameDirPath) {
                let mamePath = Helpers.getFirstExistingDirectory(mameBinaryNames, mameDirPath[0]);
